    ...
    private void Update()
    {
      ...
      // rotate player to movement direction
        ...
        //anim.SetFloat(animationFloatname, inputAmount, 0.2f, Time.deltaTime);
         ...
         anim.SetFloat("strafe", horizontal, 0.2f, Time.deltaTime);
    }
 
   
   private void FixedUpdate () {
        // if not grounded , increase down force
        if(FloorRaycasts(0,0,1.6f) == Vector3.zero)
        {
        ...
        if(FloorRaycasts(0,0, 1.6f) != Vector3.zero && floorMovement != rb.position)
